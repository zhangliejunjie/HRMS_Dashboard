...
export default function KietInterviewChip({ candidateId, open, onClose }) {
  ...
  const names = [...Array(interviewers.length)].map((_, index) => interviewers[index]?.fullname);
  ...
  const formik = useFormik({
    ...
    onSubmit: (value) => {
      ...
      console.log([...Array(inter.length)].map((_, index) => inter[index]?.id));
      axios
        .patch('http://localhost:8000/api/report/interviewers', {
          candidateId: candidateId.id,
          interviewersId: [...Array(inter.length)].map((_, index) => inter[index]?.id),
        })
        .then((res) => {
          onClose();
          dispatch(success('Assigning interviewers successfully'));
        });
    },
  });
        ...
        <FormControl>
          ...
          >
            {interviewers.map((person) => (
              <MenuItem key={person} value={person} style={getStyles(person.fullname, personName, theme)}>
                {person.fullname}
              </MenuItem>
            ))}
          </Select>
